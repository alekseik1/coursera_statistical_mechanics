...
\pythonfile{../w5/programs_lecture_5/harmonic_wavefunction_check}{Проверка аналитической формулы для $\psi_\text{harmonic} (x)$ (\texttt{hamonic\_wavefunction\_check.py})}{code:psi_harmonic_check}
...
\pythonfile{../w5/programs_lecture_5/matrix_square_harmonic}{Программа для подсчета $\rho$ в приближении Trotter decomposition(\texttt{matrix\_square\_harmonic.py})}{code:matrix_square_harmonic}
...
\pythonfile{../w5/programs_lecture_5/naive_harmonic_path}{Программа для подсчета путей в приближении Trotter decomposition для гармонического потенциала (\texttt{naive\_harmonic\_path.py})}{code:naive_harmonic_path}
